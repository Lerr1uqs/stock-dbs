...
@click.option("-a", "--adj", type=str, default = "hfq", help="结束时间")
def download_to_db(start, end, adj): # TODO: 取消一切adj的default
    '''
    下载指定时间内的所有ts数据到db中去
    '''
    logger.info(f"use {adj} adjustment")
    ...
    for tick in tqdm(StocksManager.gen_code_list()):
        # 根据当前日期和时间生成文件名
        db.download_and_dump2db(tick, start_date=start, end_date=end, adj=adj)
